...
class MainActivity : VMBaseActivity<ActivityMainBinding, MainViewModel>(),
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        // 阅读引擎: 首次启动进入引擎设置页, 之后直达书架
        runCatching {
            val p = getSharedPreferences("reading_engine", 0)
            if (!p.getBoolean("setup_shown", false)) {
                p.edit().putBoolean("setup_shown", true).apply()
                startActivity(android.content.Intent(this, io.legado.app.engine.EngineSetupActivity::class.java))
            }
        }
        upBottomMenu()
        initView()
